...
//! Source 64677fee national_team.rs; explicit shared context replaces Game.
...
pub fn process_national_team_fixtures_due(
    ...
    for (team_index, fixture_index) in due {
        ...
        game.world_history.apply_national_result(
            &super::world_cup::nation_code_of_national_team(&home_id),
            &super::world_cup::nation_code_of_national_team(&away_id),
            home_goals,
            away_goals,
            false,
        );
...
fn apply_carry_back(
    ...
    for pid in xi {
        if let Some(player) = game.players.iter_mut().find(|p| p.id == pid) {
            super::wear::apply_match_wear(player, 90, rng);
            super::wear::roll_match_injury(player, rng);
            if morale_delta != 0 {
