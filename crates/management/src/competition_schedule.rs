//! Competition calendars adapted from pinned OpenFoot Manager 64677fee.
//! Copyright (C) 2020–2026 Pedrenrique G. Guimarães and contributors.
//! SPDX-License-Identifier: GPL-3.0-or-later
//! Source scheduling rules unchanged; fixture IDs are explicit deterministic
//! coordinates and the runtime validates dates before calendar advancement.
...
// IDs derive from immutable fixture coordinates; no ambient UUID source.
fn calendar_id(parts: &[&str]) -> String {
    serde_json::to_string(parts).expect("string serialization")
}
...
pub fn generate_league(
    ...
    let league_id = calendar_id(&["league", name, &season.to_string()]);
...
pub fn generate_knockout_cup(
    ...
    let competition_id = calendar_id(&["cup", name, &season.to_string()]);
...
pub fn advance_knockout_round_with(
    ...
    if let Some(advancing) = next_round_order(&winners, &byes) {
        ...
        let round_start = date_str_to_utc(last_round_date).expect("validated fixture date")
...
pub fn generate_preseason_friendlies(
    ...
    for round in 0..rounds_to_schedule {
        ...
        for i in 0..half {
            ...
            fixtures.push(Fixture {
                id: calendar_id(&["friendly", &date, &team_ids[home_idx], &team_ids[away_idx]]),
